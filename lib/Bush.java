...
public class Bush implements DrawingObject {
    ...
    public Bush(int xPosition, int yPosition, int width, int height) {
        ...
        numOfBushClusters = random.nextInt( Math.max(1, (int) h * w / 100) , (int) h * w / 50);

        for (int i = 0; i < numOfBushClusters; i++) {
            int cWidth = random.nextInt(width / numOfBushClusters + 15, width / numOfBushClusters + 25);
            int cHeight = random.nextInt(height / numOfBushClusters + 10, height / numOfBushClusters + 20);
